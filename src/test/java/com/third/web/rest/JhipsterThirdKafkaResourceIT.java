...
import com.third.config.KafkaProperties;
...
class JhipsterThirdKafkaResourceIT {
    ...
    void setup() {
        ...
        JhipsterThirdKafkaResource kafkaResource = new JhipsterThirdKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-third-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jhipster-third-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
